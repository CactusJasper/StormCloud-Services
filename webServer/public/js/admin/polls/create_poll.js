$(() => {
    ...
    $(document).on('click', '.remove-field', (event) => {
        ...
        for(let i = fieldId + 1; i < 16; i++)
        {
            if($(`#field-${i}`)[0] != undefined)
            {
                ...
                $($($(`#field-${i}`).children()[1]).children()[0]).attr('id', `o${i - 1}`);
                $($($(`#field-${i}`).children()[1]).children()[0]).attr('name', `o${i - 1}`);
    ...
    $('#addField').click((event) => {
        ...
        if(toAdd < 16 && toAdd > 2)
        {
            ...
            html += `<td class="pt-1"><input type="text" id="o${toAdd}" name="o${toAdd}" placeholder="Option ${toAdd}" class="text-colour"/></td>`;
